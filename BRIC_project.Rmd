...
# Spanning Tests ----
ff3fm <- lm(data=spanning, formula = Strategy_RET ~ Market_RiRF + SMB + HML)

summary(ff3fm)

## FF5FM
ff5fm <- lm(data=spanning, formula = Strategy_RET ~ Market_RiRF + SMB + HML + CMA + RMW)

summary(ff5fm)

## FF3FM + MOM
ff3fm_mom <- lm(data=spanning, formula = Strategy_RET ~ Market_RiRF + SMB + HML + MOM)

summary(ff3fm_mom)

## FF5FM + MOM
ff5fm_mom <- lm(data=spanning, formula = Strategy_RET ~ Market_RiRF + SMB + HML + CMA + RMW + MOM)
...
texreg(list(ff3fm,ff3fm_mom,ff5fm, ff5fm_mom), booktabs = T, fontsize="small", custom.model.names = c("FF3FM","FF3FM + Mom", "FF5FM", "FF5FM + Mom"))


# correlation matrix between factors for the BRIC region
cor_mat <- cor(spanning[,c("SMB","HML","CMA","RMW","MOM")])

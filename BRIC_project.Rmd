...
## 6.6 Saving Statistics into a single data frame
```{r}
...
strategy_vs_bm_returns <- ggplot(data = bm_plot_df, aes(x = ym)) + 
  ...
  labs(y= "Cum. Log Returns", x = "Year", title ="Monthly Cum. Log Returns: 3FStrategy : 1-month growth vs Benchmark") + # x and y axis lables and plot title
...
overview_strategy <- ggplot(data = evolution, aes(x = ym)) + 
  ...
  labs(y= "country weight", x = "ym", title ="Country Weights by Value in the 3FStrategy : 1-month growth") + # x and y axis lables and plot title
  theme(plot.title = element_text(face="bold", hjust=0.5)) +
  scale_colour_manual(name="Country weight", 

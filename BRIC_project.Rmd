...
# Merge the B/M portfolio allocation back from July Y to June Y+1
BRIC.mfstrategy <- merge(BRIC.mfstrategy,hlpvariable2,
                       by.x=c("hcjun"),
                       by.y=c("year"),
                       all.x=T)

BRIC.mfstrategy[ , pf.value := ifelse(BM>median,"Value",(ifelse(BM<=median,"Growth",NA)))]


table(BRIC.mfstrategy$pf.value)
```
...
ret_tbl <- subset(BRIC.mfstrategy, select= c("RET.USD","Id","ym"))

# creating the 1st lag by id 
lag_1id <- shift(BRIC.mfstrategy$RET.USD,n=1)

lag_1id

# creating the 12th lag by id 
lag_12id <- shift(BRIC.mfstrategy$RET.USD,n=12)


##lag_12id <- ret_tbl %>%
##   group_by(Id) %>% 
##   mutate(shift(RET.USD,n=12), na.rm =TRUE)


ret_tbl$mon_return <-  BRIC.mfstrategy$RET.USD
...
# creating a separate dataframe for the strategy implementation
BRIC.strategy <- BRIC.maindata

# ROLLING VOLATILITY COLUMN for all stock returns

# NEW approach: creating a column for a 36 months rolling SD for the stock returns 
##BRIC.strategy <- BRIC.strategy[, vol := lag(rollapplyr(RET.USD, 36, sd, fill = NA, partial = 12),1), by =Id]


summary(BRIC.strategy$vol) 

sum(is.na(BRIC.strategy$vol)) # 2,328,303

# LOW VOLATILITY STRATEGY

# LOW VOL subset of stocks (20% of stocks with the lowest volatility)

# 20% quantile of the stocks sorted by increasing volatility
quantile(sort(BRIC.strategy$vol, decreasing =F), probs=c(0.2,0.8), na.rm = T)
# the breaking point (the 20% quantile value) is 11.06796

# splitting the range of the data in the vol column into intervals; breaks: cut points
quantiles <- cut(BRIC.strategy$vol , breaks=c(0,11.06796), include.lowest=TRUE, dig.lab=6)
summary(quantiles) # no. of rows withing the lowest 20% vol stocks: 227,121


# selecting the low vol stocks
low_vol <- subset(BRIC.strategy, vol <= 11.06796)

summary(low_vol) # row dimension: 227,121 check!

...
# delete entry for "1994-06-30" as lag-value is wrong and this data point is not needed
...
BRIC.strategy <- BRIC.monthly

# creating a time series object by stock i.e. a return panel
##BRIC.strategy.panel <- pdata.frame(x=BRIC.strategy, index = c("Id", "ym"))

##ret_panel <- BRIC.strategy.panel["RET.USD"]

##head(ret_panel)

#ret_panel <- zoo(ret_panel) you can't convert it into a zoo object

class(ret_panel)

# 36 months rolling SD for the stock returns
roll_sd_36 <- rollapply(data= zoo(BRIC.strategy$RET.USD), FUN = sd , width=36, by.column=F, na.rm=F)
...
sum(is.na(roll_sd_36)) # 1,736,313

# 24 months rolling SD for the stock returns
roll_sd_24 <- rollapply(data= zoo(BRIC.strategy$RET.USD), FUN = sd , width=24, by.column=F, na.rm=T)

roll_sd_24

sum(is.na(roll_sd_24)) # 1,837,842

# adding a volatility column to the data.frame

BRIC.strategy$vola <- roll_sd_36
...
low_vol <- subset(roll_sd_36, cumsum(roll_sd_36) >= 0.2*sort(roll_sd_36, decreasing = F))
...
Plot factor returns (performance by factor and/or by country)
Plot portfolio performance against the benchmark 

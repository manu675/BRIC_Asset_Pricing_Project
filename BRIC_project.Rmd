...
!!!!Note: If we remove less columns in our data sheet we can add them back here without any problem!!!!
## 4 minimise BRIC.yearly and BRIC.monthly to columns needed
...
# for BRIC.monthly we keep: 
BRIC.monthly <- subset(BRIC.monthly, select = c("Id","country","Date","MV","MV.USD","RET","RET.USD","ym"))
```

# 5 FF5FM Calculation
## 5.1 Adding the 1-m treasury rate to the monthly data
```{r}
...
## ---- INTERMEDIATE ---- create "working copys" of BRIC.monthly and BRIC.yearly // may be deleted later
```{r}
BRIC.monthly.df <- BRIC.monthly
BRIC.yearly.df <- BRIC.yearly
```

## 5.3 Calculate SMB and HML
...
BRIC.monthly.df[,month := month(Date)]
BRIC.monthly.df[,year := year(Date)]
BRIC.monthly.df[,hcjun := ifelse(month>=7,year,year-1)]
...
setorder(BRIC.monthly.df,Date,-MV.USD)
hlpvariable <-  BRIC.monthly.df[month==7 & !is.na(MV.USD),

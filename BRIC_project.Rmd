...
portfolio_hml <- BRIC.maindata[!is.na(pf.size) & !is.na(pf.bm)] %>% # this operator nests functions
  ...
  summarize(ret.port = weighted.mean(RiRF,
...
#6 STRATEGY II - Calculating portfolio characteristics
...
## 6.0 Weight column for BRIC.strategy
...
# Calculate GDP weights -----------

colnames(GDPData)[1] <- "hcjun"
strategy_gdp <- inner_join(x = strategy, y = GDPData, by = "hcjun")

strategy_gdp.EqualWeights <- aggregate(Id ~ ym, data = strategy_gdp, FUN=function(x) length(unique(x)))
colnames(strategy_gdp.EqualWeights)[2] <- "NumberOfStocks"

## join value weights to stocks
strategy_gdp <- inner_join(x = strategy_gdp, y = strategy_gdp.EqualWeights, by = "ym")

## calculate value weight
strategy_gdp$Weight <- 1/strategy_gdp$NumberOfStocks

## calculate value weight excess return / return
strategy_gdp$wRet <- strategy_gdp$Weight*strategy_gdp$RET.USD
strategy_gdp$wExRet <- strategy_gdp$Weight * strategy_gdp$RiRF

strategy_gdp$newWeight <- strategy_gdp$Weight * strategy_gdp$IND_w

```

## 6.1 Portfolio Statistics
...
vw <- c(strategy.retBricT,strategy.retExBricT,strategy.sdBricT,strategy.srBricT,strategy.mdBricT,strategy.teBricT,strategy.irBricT)
tw <- cbind(ew,vw)
```

### 6.1.2 Overview different weights
Run the different 6.0.x and then 6.1.1.
```{r}
ew <- c(strategy.retBricT,strategy.retExBricT,strategy.sdBricT,strategy.srBricT,strategy.mdBricT,strategy.teBricT,strategy.irBricT)
vw <- c(strategy.retBricT,strategy.retExBricT,strategy.sdBricT,strategy.srBricT,strategy.mdBricT,strategy.teBricT,strategy.irBricT)
bm <- c(benchmark.retBricT,benchmark.retExBricT,benchmark.sdBricT,benchmark.srBricT,benchmark.mdBricT,benchmark.teBricT,benchmark.irBricT)
...
- replace RiRF by RiRF.local

```{r}
# join factors and strategy ----
...
##Data_Excursus3a[,summary(lm(MOM ~ RiRF.local + SMB + HML))]
summary(lm(data=spanning, formula = wRet ~ RiRF + SMB + HML))

# Check if WML is spanned by the FF5FM
##Data_Excursus3a[,summary(lm(WML ~ RiRF.local + SMB + HML + CMA + RMW))]
summary(lm(data=Data_Excursus3a, formula = WML ~ RiRF.local + SMB + HML + CMA + RMW))

# Check if WML is spanned by the FF3FM with HML_d
##Data_Excursus3a[,summary(lm(WML ~ RiRF.local + SMB + HML.devil))]
summary(lm(data=Data_Excursus3a, formula = WML ~ RiRF.local + SMB + HML.devil))
...
##Data_Excursus3a[,summary(lm(HML ~ RiRF.local + SMB + WML))]
summary(lm(data=Data_Excursus3a, formula = HML ~ RiRF.local + SMB + WML))

##Data_Excursus3a[,summary(lm(HML.devil ~ RiRF.local + SMB + WML))]
summary(lm(data=Data_Excursus3a, formula = HML.devil ~ RiRF.local + SMB + WML))

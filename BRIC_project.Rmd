...
hlpvariable2 <- BRIC.strategy[month==7, .(median = quantile(BM , probs = c(0.5), na.rm=T)),by=year]
...
## 6.6 Effective N calculation
```{r}
#library(plyr)
EFF_N<- ddply(.data = strategy, .(Id, year), 
                summarize,  # Calculate an inverse HHI
                invHHI = sum(table(Weight))^2 / sum(table(Weight)^2))
strategy.eff_n <- (sum(EFF_N$invHHI) / (nrow(EFF_N))) #-3.58

EFF_N_bm<- ddply(.data = BRIC.benchmark, .(Id, year), 
                summarize,  # Calculate an inverse HHI
                invHHI = sum(table(ValueWeight))^2 / sum(table(ValueWeight)^2))
benchmark.eff_n <-mean(EFF_N_bm$invHHI) #-9.76
benchmark.eff_n
```
...
Table/Plot:
1) Strategy BRIC vs Countries stand alone // JOHANNES ## Tables done
2) Strategy parts stand alone and/or 2 combined vs 3ple sort // Johannes ## Tables done
...
overview_strategy <- ggplot(data = evolution, aes(x = ym)) + 
  ...
  labs(y= "country weight", x = "ym", title ="Country Weights by Value in the 3FStrategy") + # x and y axis lables and plot title
  theme(plot.title = element_text(face="bold", hjust=0.5)) +
  scale_colour_manual(name="Country weight", 
...
# 8.4.3 GDP evolution as comparison ----
# load data sheet from French's website
...
```
# 9.1 Other statistics
```{r}
deStats <- BRIC.maindata
deStats <- subset(deStats,pf.size == "Big")
# Total number of firms
# BRIC
length(unique(deStats$Id))
# countrywise
aggregate(Id ~ country.x, data = deStats, FUN=function(x) length(unique(x)))

# minimum number of firms
zu <- aggregate(Id ~ ym, data = deStats, FUN=function(x) length(unique(x)))
min(zu$Id)
mean(zu$Id)
max(zu$Id)
...
sum(su_BRA$MV.USD)/sum(deStats$MV.USD)
mean(su_BRA$MV.USD)
median(su_BRA$MV.USD)
su_RUS <- subset(su,country.x == "RUS")
sum(su_RUS$MV.USD)/sum(deStats$MV.USD)
mean(su_RUS$MV.USD)
median(su_RUS$MV.USD)
su_IND <- subset(su,country.x == "IND")
sum(su_IND$MV.USD)/sum(deStats$MV.USD)
mean(su_IND$MV.USD)
median(su_IND$MV.USD)
su_CHN <- subset(su,country.x == "CHN")
sum(su_CHN$MV.USD)/sum(deStats$MV.USD)
mean(su_CHN$MV.USD)
median(su_CHN$MV.USD)
...
##5.3 Filtering for multiple strategy requirements
```{r}
# filtering for growth stocks

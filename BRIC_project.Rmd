...
## 1.1 Save raw data and adjust date range
```{r}
#0 save raw data
...
```
## 1.2 BRIC.monthly data preparation
...
# MOM (recent previous month not included)
...
# MOC (recent previous month included)
...
# 1.2.4 Adding ordering columns MV.USD.June and MV.December ----
...
# 1.2.6 Quarterly rebalancing information ----
...
# 1.2.9 Adding 1-month t-bill Rate ----
...
```
## 1.3 BRIC.yearly data preparation and value calculation

```{r}
# 1.3.1 the BRIC.yearly panel here is still balanced! We need to add a column for total assets the year before ----
...
# 1.3.2 book value / equity and related ----
...
```
# 2 Merge monthly stock market data and yearly accounting data
```{r}
...
## 2.1 Breakpoint for country stand-alone evaluation
If we want to calculate factors or anything else for countries stand-alone we will have to filter them here!
For multiple trys run 2 again, to reset BRIC.maindata.
```{r}
#BRIC.maindata <- filter(BRIC.maindata,BRIC.maindata$country.x == "BRA")
#BRIC.maindata <- filter(BRIC.maindata,BRIC.maindata$country.x == "IND")
#BRIC.maindata <- filter(BRIC.maindata,BRIC.maindata$country.x == "RUS")
#BRIC.maindata <- filter(BRIC.maindata,BRIC.maindata$country.x == "CHN")
```
## 2.2 Determine size Breakpoints (Benchmark and SMB)
```{r}
...
# 3 FF5FM + MOM & Benchmark 
## 3.1 Benchmark and Market
...
```
## 3.2 Determine other breakpoints
```{r}
...
```
## 4.4 Quarterly rebalancing
Leave this chunk out, if you want to get monthly rebalancing // RUN only, when doing quarterly rebalancing
```{r}
...
WHEN using equal weights run 5.2, which is adjusted to the bottom of this markdown file after 8 and don't run this one!
...
# 6 Spanning tests (factor exposure)
```{r}
library(xtable)
library(texreg)
...
# 7 Plots and Visualisation

## 7.1 Benchmark vs Strategy
```{r}
...
strategy_vs_bm_returns <- ggplot(data = bm_plot_df, aes(x = ym)) + 
  ...
  labs(y= "Cum. Log Returns", x = "Year", title ="      Mon. Cum. Log Returns: 3FStrategy : [Strategy] vs Benchmark") + # x and y axis lables and plot title
...
dev.off()
```
## 7.2 Evolution of MarketCap Weights
...
overview_strategy <- ggplot(data = evolution, aes(x = ym)) + 
  ...
  labs(y= "country weight", x = "ym", title ="Country Weights by Value in the 3FStrategy : [Strategy]") + # x and y axis lables and plot title
  theme(plot.title = element_text(face="bold", hjust=0.5)) +
...
## 7.4 Strategy Comparison (3factor, 2factor, 1factor)
```{r}
# monthly
# load datasheet
load("allStrategies.RData")
quarterly <- allStrategies

# subset on most important strategies
allStrategies <- allStrategies[1:18,]
allStrategies$Strategy <- c("Benchmark","MOM","MOC","VAL","GRO","VOL","MOM_VAL","MOM_GRO","MOM_VOL","MOC_VAL","MOC_GRO","MOC_VOL","VAL_VOL","GRO_VOL","MOM_VAL_VOL","MOM_GRO_VOL","MOC_VAL_VOL","MOC_GRO_VOL")


# saving the plot as a jpeg file
jpeg("risk_return_factors.jpeg", width = 800, height = 600) # height and width can be chosen as your wish 
# Zooming in with xlim/ylim
plot_combinations <- ggplot(data = allStrategies, aes(x = StandardDeviation, y = Return, color= Strategy)) +
  geom_point(size = 3, alpha = 0.9, shape = 20) +
  geom_text(color= "black", aes(label=Strategy,label=""),show_guide=F,hjust="inward",vjust="inward") +
  labs(y= "Strategy Return (in % p.a.)", x = "Volatility (SD in % p.a.)", title ="Risk-return space of factor combinations (monthly rebalancing)") + # x and y axis lables and plot title  
theme(plot.title = element_text(face="bold", hjust=0.5)) + theme(legend.position="none") # remove legend
...
quq <- quq[19:35,]
quarterly <- rbind(quarterly,quq)
quarterly$Strategy <- c("Benchmark","MOM","MOC","VAL","GRO","VOL","MOM_VAL","MOM_GRO","MOM_VOL","MOC_VAL","MOC_GRO","MOC_VOL","VAL_VOL","GRO_VOL","MOM_VAL_VOL","MOM_GRO_VOL","MOC_VAL_VOL","MOC_GRO_VOL")


# saving the plot as a jpeg file
jpeg("risk_return_factors_quarterly.jpeg", width = 800, height = 600) # height and width can be chosen as your wish 
# Zooming in with xlim/ylim
plot_comb <- ggplot(data = quarterly, aes(x = StandardDeviation, y = Return, color= Strategy)) +
  geom_point(size = 3, alpha = 0.9, shape = 20) +
  geom_text(color= "black", aes(label=Strategy,label=""),show_guide=F,hjust="inward",vjust="inward") +
  labs(y= "Strategy Return (in % p.a.)", x = "Volatility (SD in % p.a.)", title ="Risk-return space of factor combinations (quarterly rebalancing)") + # x and y axis lables and plot title  
theme(plot.title = element_text(face="bold", hjust=0.5)) + theme(legend.position="none") # remove legend

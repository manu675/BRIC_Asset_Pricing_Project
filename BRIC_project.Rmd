...
plot_combinations <- ggplot(data = factor_plot_df, aes(x = Volatility, y = Return, label=Strategies)) +
  geom_point(color ="blue", size = 3, alpha = 0.9, shape = 1) +
  geom_text(aes(label=Strategies),hjust=0.5,vjust=-0.5) +
  labs(y= "Strategy Return", x = "Risk (Volatility)", title ="Risk return space of factor combinations") + # x and y axis lables and plot title 
  theme(plot.title = element_text(face="bold", hjust=0.5))
  #scale_colour_manual(name="Factor combination", 
  #                    breaks = c(),
  #                    labels= factor_plot_df$Strategies, # legend label names
  #                    values = c("red", "green")) 

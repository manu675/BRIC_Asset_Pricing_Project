...
BRIC.monthly <- BRIC.monthly %>%
  drop_na(RET.USD,MV,MV.USD,LMV.USD,MV.USD.June,volatility,momentum)
```
...
# rename columns
colnames(benchmark_BRIC)[2] <- "YearlyExcessReturn"
colnames(benchmark_BRIC)[4] <- "MonthlyExcessReturn"
colnames(benchmark_Country)[3] <- "YearlyExcessReturn"
colnames(benchmark_Country)[5] <- "MonthlyExcessReturn"

# plot
a <- ggplot(data = benchmark_BRIC) + geom_line(aes(x = ym, y = MonthlyExcessReturn)) + geom_line(aes(x = ym, y = YearlyExcessReturn, color="red"))
a
```
...
# per year
strategy.retBricY <- aggregate(RMRF ~ hcjun, data = BRIC.strategy,FUN = mean)
...
overview <- ggplot(data = GDPData) + geom_line(aes(x = year, y = BRA_w)) + geom_line(aes(x = year, y = CHN_w, color="red")) + geom_line(aes(x = year, y = IND_w, color="blue")) + geom_line(aes(x = year, y = RUS_w, color="green"))
overview

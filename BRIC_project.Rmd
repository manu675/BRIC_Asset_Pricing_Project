...
BRIC.benchmark$wExRet <- BRIC.benchmark$ValueWeight * BRIC.benchmark$RMRF
...
## calculate value weight
strategy$ValueWeight <- strategy$LMV.USD/strategy$TotalValue

## calculate value weight excess return / return
strategy$wRet <- strategy$ValueWeight*strategy$RET.USD
strategy$wExRet <- strategy$ValueWeight * strategy$RMRF
```
### 6.0.3 GDP Weights
```{r}
# assign a new working variable
...
strategy_gdp$wExRet <- strategy_gdp$Weight * strategy_gdp$RMRF
...
# per country returns
# per year
strategy.retCountryY <- aggregate(RMRF ~ country.x + hcjun, data = strategy,FUN = mean)
strategy.retCountryY$RMRF <- strategy.retCountryY$RMRF * 12 #annualizing the returns
# per month
strategy.retCountryM <- aggregate(RMRF ~ country.x + ym + hcjun, data = strategy,FUN = mean)
...
intermediate_df <- strategy %>% select(c("Id","year"))
...
intermediate_df <- strategy %>% select(c("Id","year","country.x"))
...
intermediate_df <- strategy %>% select(c("Id","year","volatility"))

# annualized volatility by stock per time point
strategy$vol_an <- strategy$volatility*sqrt(12)
